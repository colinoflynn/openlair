...
wwd_result_t host_platform_init( void )
{
    ...
    sdhc_capability_t mycapability;
    ...
    sdhcConfig.dmaMode = kSDHC_DmaModeNo;
    sdhcConfig.readWatermarkLevel = 128;
    sdhcConfig.writeWatermarkLevel = 128;
    src_clk = CLOCK_GetCoreSysClkFreq();
    SDHC_Init(SDHC, &sdhcConfig); //Initialized SDHC by starting clk and enabling interrupt status
    SDHC_GetCapability(SDHC,&mycapability);
    SDHC_SetSdClock(SDHC,src_clk,400000);
...
{
#if defined ( WICED_USE_WIFI_RESET_PIN )
    ( reset_asserted == WICED_TRUE ) ? platform_gpio_output_high( &wifi_control_pins[ WWD_PIN_RESET ] ) : platform_gpio_output_low( &wifi_control_pins[ WWD_PIN_RESET ] );
#else
...
#else
//If using one of WICED avail platforms
wwd_result_t host_platform_init( void )
{
    host_platform_deinit_wlan_powersave_clock( );
